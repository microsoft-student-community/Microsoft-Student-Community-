...
const FuzzyText = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Skip document.fonts.load to avoid hanging indefinitely if the font isn't explicitly found
        ...
        if (typeof fontSize === 'number') {
          ...
          numericFontSize = parseFloat(computedSize) || 80; // Fallback to 80px if NaN
        ...
        const textBoundingWidth = Math.max(Math.ceil(letterSpacing !== 0 ? totalWidth : actualLeft + actualRight), metrics.width || 100);
        const tightHeight = Math.max(Math.ceil(actualAscent + actualDescent), numericFontSize || 100);
  ...
  if (error) {
    return <div style={{ color: 'red', fontSize: '20px' }}>FuzzyText Error: {error}</div>;
  }
