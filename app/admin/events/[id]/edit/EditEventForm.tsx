...
export default function EditEventForm({ event }: { event: any }) {
  ...
  return (
    <div className="bg-slate-900  border border-slate-800 rounded-md p-8 mb-8 ">
      <form onSubmit={handleEditEvent} className="flex flex-col gap-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="flex flex-col gap-2">
            ...
            <input type="text" name="title" required defaultValue={event.title} className="p-3 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[13px] font-semibold text-[#a1a1aa] uppercase tracking-wider">Date</label>
            <input type="date" name="date_start" required defaultValue={defaultDate} className="p-3 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[13px] font-semibold text-[#a1a1aa] uppercase tracking-wider">Status</label>
            <select name="status" defaultValue={event.status} className="p-3 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500 appearance-none">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select name="type" defaultValue={event.type || 'hackathon'} className="p-3 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500 appearance-none">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input type="text" name="location" defaultValue={event.location || ''} className="p-3 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500" />
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-[13px] font-semibold text-[#a1a1aa] uppercase tracking-wider">Event Description</label>
          <textarea name="description" rows={3} required defaultValue={event.description || ''} className="p-4 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500"></textarea>
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-[13px] font-semibold text-[#a1a1aa] uppercase tracking-wider">Maximum Capacity (Optional)</label>
          <input type="number" name="max_capacity" min="1" defaultValue={event.max_capacity || ''} className="p-4 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500" />
          <span className="text-[10px] text-slate-500">If set, any registrations past this limit will automatically be placed on a Waitlist.</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="file" name="image" accept="image/*" className="w-full bg-slate-950 border border-slate-800 rounded-md px-4 py-3 text-white focus:outline-none focus:border-blue-500 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-500/20 file:text-blue-400 hover:file:bg-blue-500/30 transition-all cursor-pointer" />
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea 
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-md p-4 text-slate-100 font-mono text-xs focus:outline-none focus:border-yellow-500"
          ></textarea>
        </div>

        {/* Gallery Upload Section */}
        <div className="flex flex-col gap-4 mt-2 p-5 bg-slate-950 border border-slate-800 rounded-md">
          <div className="flex justify-between items-end">
            <label className="text-[13px] font-bold text-[#a1a1aa] uppercase tracking-wider">Event Photo Gallery</label>
            <span className="text-[10px] text-slate-500 normal-case font-normal">Upload photos from the event (especially useful after completion).</span>
          </div>
          ...
          <input 
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-md px-4 py-3 text-white focus:outline-none focus:border-blue-500 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-500/20 file:text-blue-400 hover:file:bg-blue-500/30 transition-all cursor-pointer" 
          />
        ...
        <div className="mt-4 p-5 bg-slate-950 border border-slate-800 rounded-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-3">
              ...
              <label className="flex items-center gap-3 p-3 bg-white border border-slate-800 rounded-md cursor-pointer hover:bg-slate-800 transition-colors">
            ...
            <div className="flex flex-col gap-3 border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-4">
              <label className="flex items-center gap-3 cursor-pointer">
              ...
              {allowTeamsToggle && (
                <div className="flex items-center gap-3 animate-in fade-in slide-in-from-left-2 duration-200">
                  <span className="text-sm text-slate-400">Max Team Size:</span>
                  <input type="number" name="max_team_size" defaultValue={event.form_requirements?.max_team_size || 3} min={2} max={10} className="w-20 p-2 bg-slate-950 border border-slate-800 rounded-lg text-white text-center focus:outline-none focus:border-blue-500" />
                </div>
              )}
            </div>
          </div>
            ...
            <button type="button"
              ...
              className={`px-5 py-2.5 rounded-md text-sm font-bold transition-all border ${
                eventPricingType === 'free'
                  ...
                  : 'bg-white text-slate-400 border-slate-800 hover:bg-white'
              }`}>
            ...
            <div className="animate-in fade-in slide-in-from-top-2 duration-200 space-y-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="number" name="registration_fee" min="1" step="1" required={eventPricingType === 'paid'}
                  ...
                  placeholder="e.g. 200"
                  className="p-3 bg-slate-950 border border-slate-800 rounded-md text-white focus:outline-none focus:border-blue-500" />
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <button type="submit" className="px-8 py-3 bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 rounded-md font-bold transition-all shadow-lg text-white">Save Changes</button>
        </div>
      </form>
    </div>
  )
