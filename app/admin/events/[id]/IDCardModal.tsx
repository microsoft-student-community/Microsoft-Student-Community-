...
export default function IDCardModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950 print:bg-white print:">
      
      {/* Non-Printable Modal Controls */}
      <div className="absolute top-0 w-full p-4 flex justify-between items-center bg-slate-900 border-b border-slate-800 print:hidden">
        <h2 className="text-xl font-bold text-slate-100">ID Card Preview ({registrations.length})</h2>
        <div className="flex gap-4">
          <button 
            onClick={handlePrint}
            className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-slate-100 font-bold rounded-md transition-colors flex items-center gap-2"
          >
            <i className="fas fa-print"></i> Print Badges
          </button>
          <button 
            onClick={onClose}
            className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold rounded-md transition-colors border border-slate-700"
          >
            Close
          </button>
        </div>
