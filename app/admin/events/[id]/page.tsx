    ...
    <div className="flex h-screen bg-[#fdfaf6] text-[#f4f4f5] font-sans overflow-hidden">
      <main className="flex-1 overflow-y-auto p-4 md:p-10 relative">
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          <Link href="/admin" className="text-black/40 hover:text-black transition-colors flex items-center gap-2 text-sm font-semibold mb-8">
            <i className="fas fa-arrow-left"></i> Back to Dashboard
          </Link>

          <div className="bg-[#f4f4f0]/60 backdrop-blur-xl border border-4 border-black rounded-none p-8 mb-8  flex justify-between items-center">
            <div>
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-bold uppercase tracking-wider text-black/40">Total Registrations</p>
            </div>
          </div>

          {/* Quick Actions */}
          <div className="mb-8 flex flex-wrap gap-4">
            <Link href={`/admin/events/${event.slug || id}/scanner`} className="px-6 py-3 bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500 hover:text-black rounded-none font-bold transition-colors flex items-center gap-3">
              <i className="fas fa-qrcode"></i> Open Live Scanner
            </Link>
            {userRole === 'admin' && (
              <Link href={`/admin/events/${event.slug || id}/edit`} className="px-6 py-3 bg-[#E0E0E0] text-black/80 border border-4 border-black hover:bg-white/10 hover:text-black rounded-none font-bold transition-colors flex items-center gap-3">
